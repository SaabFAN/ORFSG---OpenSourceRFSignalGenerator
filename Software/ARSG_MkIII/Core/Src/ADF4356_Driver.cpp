...
uint8_t ADF4356_Driver::FPGA_Setup(unsigned char directMode, long updateFreq,
...
uint8_t ADF4356_Driver::FPGA_START(unsigned char start) {// Signals the FPGA to START or STOP sending the data-buffer to the ADF4355
...
void ADF4356_Driver::FPGA_WriteData(float frequency, float phase, int amplitude) {
...
void ADF4356_Driver::FPGA_DUMP(int address) {//Dumps the FPGA-Registers and Memory at the specified FPGA Memory-Address

}

void ADF4356_Driver::SetFrequency(double frequency) {
	ConvertFreq(frequency);
	WriteData();
	// ADF4355_WriteDataLite(FREQ_UPDATE);
}

void ADF4356_Driver::SetPhase(float phase) {
...
void ADF4356_Driver::WriteDataLite(uint8_t mode) {	// Updates only the Registers that
	switch (mode) {
...
uint32_t ADF4356_Driver::ReadRegister(uint8_t reg){
	// TODO: Configure the Chip so that the MUX-Pin becomes MISO.
	// TODO: READ Data from the Chip via the repurposed MUX-Pin.
}
